...
- Folder ClickUp (Projetos Ativos `<ID_CLICKUP>`, Sprint `<ID_CLICKUP>`)
- Listas excluídas (triagem `<ID_CLICKUP>`, gestão `<ID_CLICKUP>`)
...
Scripts (`tools/`):
- `integrations/clickup_pull_projetos_ntics.py` — pull do folder `<ID_CLICKUP>`
